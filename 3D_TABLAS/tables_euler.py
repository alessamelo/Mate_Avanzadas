...
output_dir = os.path.join(base_dir, "euler", "sensibilidad")
...
# === Simulación base ===
t_base, y_base = euler_system(f, 0, y0_values[0], T, h)
...
for y0 in y0_values:
    ...
    nfe = int(T / h)  # Euler: 1 evaluación por paso
    sensitivity_df.loc[len(sensitivity_df)] = [y0[2], h, nfe] + deltas

# === Guardar CSV ===
csv_path = os.path.join(output_dir, "euler_sensibilidad_con_NFE.csv")
