...
def main_rk4_system(f, y0, T, h):
    ...
    plt.plot(t_vals, y_vals[:, 2], label='Z (RK4)', color="#f0a8cc", linewidth=1)
    plt.xlabel("t")
    plt.ylabel("Functions of the system")
    plt.legend()
    plt.title(f"Runge-Kutta 4 for Lorenz System\nh={h}, y0_z={y0[2]}")
    plt.savefig(os.path.join(rk4_dir, "rungen_lorenz", f"{filename_base}.png"))
    plt.close()
