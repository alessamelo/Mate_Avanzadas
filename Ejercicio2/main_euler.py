...
# === Funciones ===
def main_euler_system(f_generator, y0, u, h):
    if u == 10:
        T = 50
    elif u >= 50:
        T = 200
    ...
    plt.legend()
    plt.title(f"Euler Method for Oscillator System\nh={h}, y0_x={y0[0]}, y0_y={y0[1]}, u={u}")
    plt.savefig(os.path.join(euler_dir, "euler_oscillator", f"{filename_base}.png"))
    plt.close()
